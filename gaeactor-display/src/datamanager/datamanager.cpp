    ...
    m_binary_websocket_server_ex(nullptr)
{
    ...
    updatereviewsrecorddir("lavic_desktop_custom_"+dateStr);


	//	UINT32 maxThreads = 0;
	//	UINT32 usingThreads = 100;
	//#ifdef WIN32 //Windows �¶���
	//	SYSTEM_INFO systemInfo;
	//	GetSystemInfo(&systemInfo);
	//	maxThreads = systemInfo.dwNumberOfProcessors * 4;
	//#else
	//	maxThreads = sysconf(_SC_THREAD_THREADS_MAX);
	//#endif
	//	usingThreads = usingThreads > (maxThreads * 3 / 4) ? (maxThreads * 3 / 4) : usingThreads;
	//
	//	std::cout << "++++++++++++++++ maxThreads: " << maxThreads << " using threads: " << usingThreads << " ++++++++++++++++" << std::endl;

	//	m_pThreadPool = new TaskThreadPool::ThreadPool(20);

    load_ive_model_path();
#ifdef CALC_PATH

	m_pDijkstra = new Dijkstra();
	m_pDijkstra->Create_graph(20000);
#endif // CALC_PATH

	initHttp();

//    //QString url = "https://images.pexels.com/photos/20888053/pexels-photo-20888053.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1";
//    QString url = "http://192.168.31.230:9000/lavic/8ef89bd0-dd1b-4687-931c-6c7f83f9c7f5";
////	QString url = "http://p6.itc.cn/q_70/images03/20200925/0a0f440227a24e9e8bd3cb8188329bb6.jpeg";
...
//
//    trans_log("操作：仿真数据 ", std::stringstream() << filename.toStdString(), std::stringstream() << "开始下载");
//    if (pHttpClient() && !pHttpClient()->requeset_sim_data(url.toStdString(), filename.toStdString()))
//    {
//        trans_log("操作：仿真数据 ", std::stringstream() << filename.toStdString(), std::stringstream() << "下载失败");
//    }
//    int cc = 0;
//    cc = 1;
...
#else
    m_hDataDealThread = new stdutils::OriThread(std::bind(&DriverPoolProcessor::data_deal_thread_func,this,std::placeholders::_1),\
                                                nullptr,\
                                                2);
#endif
    ...
    auto storage_instance = initStorage(m_log_dir_simname.toStdString()+"/data.db");
    {
        ...
        for(auto itor  = m_saveFlightPlanConfLogs.begin(); itor != m_saveFlightPlanConfLogs.end(); itor++)
        {
            ...
            auto count = storage_instance.count<FlightPlanConfLog>(
                where(c(&FlightPlanConfLog::flightid) == _FlightPlanConfLog.flightid));
            if (count == 0)
            {
                // make the insertion
...
void DataManager::load_ive_model_path()
{
    std::function<void (const QString &)> findTrajectoryFolders=[&](const QString &path)
    {
        ...
        for (const QFileInfo &entry : entries)
        {
            ...
            else
            {
                ...
                QString absolutePath = entry.absolutePath();

                QString filename = entry.fileName();
                if(/*filenamepath.endsWith(".fbx")|| */ filenamepath.endsWith(".ive"))
                {
                    std::cout<<" append model "<<absolutePath.toStdString()<<" "<<filenamepath.toStdString()<<"\n";
                    if(m_ive_paths.find(absolutePath) == m_ive_paths.end())
                    {
                        m_ive_paths.insert(std::make_pair(absolutePath,std::make_tuple(m_ive_paths.size(), QList<QString>())));
                    }
                    std::get<1>(m_ive_paths[absolutePath]).push_back(filenamepath);
                }
            }
        }
    };

//    QString savedir = QCoreApplication::applicationDirPath() + "/res/osgearth/model/W";
    QString savedir = QCoreApplication::applicationDirPath() + "/res/osgearth/model/M";

    if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
    {
        savedir = QCoreApplication::applicationDirPath() + "/res/osgearth/model/M";
    }
    findTrajectoryFolders(savedir);

    std::cout<<" append model ------------------"<<m_ive_paths.size()<<"\n";
}

QString DataManager::getModelPath(const UINT64 &agentid)
{
    if(!m_AGENT_ive_paths.contains(agentid))
    {
        auto generateRandomNumber=[](int max)->int {
            int max_ = (std::max)(0,max-1);
            // 创建随机数生成器
            std::random_device rd;
            std::mt19937 gen(rd());

            // 创建一个在0到255之间的均匀分布
            std::uniform_int_distribution<> dis(0, max_);

            // 生成随机数
            return dis(gen);
        };
        int c = generateRandomNumber(m_ive_paths.size());

        std::cout<<" alloc model "<<agentid<<" "<<m_ive_paths.size()<<" "<<c<<" \n";
        auto itor = std::find_if(m_ive_paths.begin(),
                                 m_ive_paths.end(),[&c](const std::map<QString,std::tuple<int,QList<QString>>>::value_type& vt)
                                 {
                                     return std::get<0>(vt.second)==c;
                                 });
        if(itor != m_ive_paths.end())
        {
            int d = generateRandomNumber(std::get<1>(itor->second).size());
            m_AGENT_ive_paths.insert(agentid, std::get<1>(itor->second).at(d));

            std::cout<<" alloc model "<<agentid<<" "<<std::get<1>(itor->second).at(d).toStdString()<<"\n";
        }
        return m_AGENT_ive_paths[agentid];
    }
    else
    {
        return QString();
...
{
    {
        ...
        QString fileName = m_log_dir_simname+"/simlogs.xlsx";
        ...
        for (auto itor = m_recordruntimelogs.begin(); itor != m_recordruntimelogs.end(); itor++)
        {
            ...
            _Xlsx.write("A" + QString::number(icount+1), _infoLog.id);
