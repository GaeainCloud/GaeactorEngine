...
//    mProjection.reset(new QGVProjectionGCJ02());
...
{
    Q_EMIT mapMouseRelease(mapToProj(event->pos()));
    QWidget::mouseReleaseEvent(event);
}

void QGVMap::mouseDoubleClickEvent(QMouseEvent *event)
{
    Q_EMIT mapMouseDoubleClick(mapToProj(event->pos()));
    QWidget::mouseDoubleClickEvent(event);
}

void QGVMap::wheelEvent(QWheelEvent *event)
{
    Q_EMIT mapwheelEvent(event);
    QWidget::wheelEvent(event);
}
void QGVMap::keyPressEvent(QKeyEvent *event)
{
    Q_EMIT mapKeyPressEvent(event);
    QWidget::keyPressEvent(event);
}
void QGVMap::keyReleaseEvent(QKeyEvent *event)
{
    Q_EMIT mapKeyReleaseEvent(event);
