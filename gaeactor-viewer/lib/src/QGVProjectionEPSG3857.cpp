...
{
    QGV::GeoPos toplefttmp = projToGeo(projRect.topLeft());
    QGV::GeoPos bottomrighttmp = projToGeo(projRect.bottomRight());
    return QGV::GeoRect(toplefttmp, bottomrighttmp);
}
...
double QGVProjectionEPSG3857::geodesicMeters(const QGV::GeoPos &geoPos1, const QGV::GeoPos &geoPos2) const
{
    const double latitudeArc = (geoPos1.latitude() - geoPos2.latitude()) * M_PI / 180.0;
    const double longitudeArc = (geoPos1.longitude() - geoPos2.longitude()) * M_PI / 180.0;
    const double latitudeH = qPow(sin(latitudeArc * 0.5), 2);
