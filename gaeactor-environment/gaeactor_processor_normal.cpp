...
void HexIdexInfo::updateIntersect()
{
    ...
    if(m_eHexidexStatus == gaeactorenvironment_normal::E_HEXINDEX_STATUS_ALL)
    {
        ...
        gaeactorenvironment_normal::H3IndexBufferManager::getInstance().appendIntersect(m_h3Index, std::move(interscet));
    }
    else
    {
        gaeactorenvironment_normal::H3IndexBufferManager::getInstance().removeIntersect(m_h3Index);
    }
}
...
HexIdexInfo* H3IndexBufferManager::getCellBufferByHex(const H3INDEX& h3)
{
    ...
    if(hexindexinfo == nullptr)
    {
        ...
        hexindexinfo->init(h3, std::move(h3cellinfo));
        {
            QMutexLocker locker(&m_h3CellBuffer_mutex);
...
{
    return gaeactorenvironment_normal::H3IndexBufferManager::getInstance();
}
...
void GaeactorProcessor::clear_hexindex_entity(const TYPE_ULID& uildsrc)
{
	...
	{
        gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_entity(uildsrc, entityinfo_itor->second.m_h3Index, E_DISPLAY_MODE_ENTITY, true);
...
//				if (entityinfo_itor->second.m_bValid)
				{
                    ...
                    gaeactorenvironment_normal::H3IndexBufferManager::getInstance().lockIntersect();
                    gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_entity(uildsrc, entityinfo_itor->second.m_h3Index, E_DISPLAY_MODE_ENTITY, true);
				}
				entityinfo_itor->second.m_bValid = false;
			}
			...
			if (!entityinfo_itor->second.m_bValid)
			{
                ...
                gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_entity(uildsrc, h3, E_DISPLAY_MODE_ENTITY, false);
                if(bUpdateRemoveAppend)
                {
                    gaeactorenvironment_normal::H3IndexBufferManager::getInstance().unlockIntersect();
                }
			}
			bNewAppend = false;
		}
	...
	bool bNewAppend = true;
	{
		...
		if (sensorInfo_itor != m_sensorInfosMap.end() && sensorInfo_itor->second)
		{
			...
			if (nullptr != ptagHexindexlistSensorInfo)
            {
				...
				{
                    gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_sensor(sensorulid, sensingmediaid, newitem, E_DISPLAY_MODE_WAVE, false);
				}
			...
			else
			{
				...
				for (auto h3 : hexidxslist)
				{
                    ...
                    gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_sensor(sensorulid, sensingmediaid, h3, E_DISPLAY_MODE_WAVE, false);
				}
				if (!hexidxslist.empty())
				{
                                               ...
                                               POLYGON_LIST &&_polygon)
{
	...
	if(bNewAppend)
	{
		...
		for (auto _hexidxs_item : _hexidxs)
		{
			const H3INDEX& h3 = _hexidxs_item.PARAM_seq_hexidx_element;
            {
                UINT8 res = (UINT8)H3_GET_RESOLUTION(h3);

                QWriteLocker locker(&m_resolutions_count_map_mutex);
                auto _resolutions_count_map_itor = m_resolutions_count_map.find(res);
                if (_resolutions_count_map_itor != m_resolutions_count_map.end())
                {
                    m_resolutions_count_map.insert(std::make_pair(res, 1));
                ...
                {
                    _resolutions_count_map_itor->second = _resolutions_count_map_itor->second + 1;
                }
            }
			hexindexlistensorinfo->append_hexindexinfo(h3, currentTimeStamp, _hexidxs_item);
            gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_sensor(sensorulid, sensingmediaid, h3, E_DISPLAY_MODE_WAVE, false);
...
void GaeactorProcessor::force_cover_update_hexindex_sensor_h3list_ex(const TYPE_ULID &sensorulid, const TYPE_ULID &sensingmediaid, const HEXIDX_ARRAY &hexidxslist, tagHexindexlistSensorInfo *ptagHexindexlistSensorInfo)
{
    ...
    {
        gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_sensor(sensorulid, sensingmediaid, invaliditem, E_DISPLAY_MODE_WAVE, true);
    }
...
#if 1
    std::unordered_map<H3INDEX, tagIntersectInfo> intersectInfos = gaeactorenvironment_normal::H3IndexBufferManager::getInstance().getIntersectInfos();
...
#else
    {
        QMutexLocker IntersectInfoMap_update_mutex_locker(gaeactorenvironment_normal::H3IndexBufferManager::getInstance().IntersectInfoMap_update_mutex());
        QReadLocker IntersectInfoMap_mutex_locker(gaeactorenvironment_normal::H3IndexBufferManager::getInstance().IntersectInfoMap_mutex());

        auto intersectInfos_itor = gaeactorenvironment_normal::H3IndexBufferManager::getInstance().IntersectInfoMap().begin();
        while (intersectInfos_itor != gaeactorenvironment_normal::H3IndexBufferManager::getInstance().IntersectInfoMap().end())
        {
            std::unordered_map<QPair<TYPE_ULID, TYPE_ULID>, bool> &sensorlist = intersectInfos_itor->second.sensorlist;
            std::unordered_map<TYPE_ULID, bool> &entitylist = intersectInfos_itor->second.entitylist;
...
void GaeactorProcessor::refresh_events_by_cores(const TYPE_ULID &entityid, const H3INDEX &h3Indexsrc,const FLOAT64 &hgt,const transdata_entityposinfo& eninfo,IDENTIFI_EVENT_INFO& identifi_event_info)
{
    ...
    //获取当前实体存在的碰撞信息
    std::unordered_map<H3INDEX, tagIntersectInfo> intersectInfos = gaeactorenvironment_normal::H3IndexBufferManager::getInstance().getIntersectInfos_by_cores(entityid);
...
void GaeactorProcessor::refresh_events_by_sensors(const TYPE_ULID &sensorid, const TYPE_ULID &sensingmedia_id, const HEXIDX_HGT_ARRAY &hexidxslistret,IDENTIFI_EVENT_INFO& identifi_event_info)
{
    ...
    //获取实体感知域存在的碰撞信息
    std::unordered_map<H3INDEX, tagIntersectInfo> intersectInfos = gaeactorenvironment_normal::H3IndexBufferManager::getInstance().getIntersectInfos_by_sensors(sensorid, sensingmedia_id);
...
void GaeactorProcessor::tagHexindexSensorInfo::clear_hexindex_sensor(const TYPE_ULID &sensorulid, const TYPE_ULID &sensingmediaid)
{
	...
	if (_sensor_hexidx_info_map_itor != this->m_sensor_hexidx_info_map.end() && _sensor_hexidx_info_map_itor->second)
	{
		...
		{
            gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_sensor(sensorulid, sensingmediaid, clearitem, E_DISPLAY_MODE_WAVE, true);
		}
	}
}
...
void GaeactorProcessor::tagHexindexSensorInfo::clear_hexindex_sensor_by_hexidexlist(const TYPE_ULID &sensorulid, const TYPE_ULID &sensingmediaid, const HEXIDX_ARRAY &hexidxslist)
{
	...
	if (_sensor_hexidx_info_map_itor != this->m_sensor_hexidx_info_map.end() && _sensor_hexidx_info_map_itor->second)
	{
		...
		{
            gaeactorenvironment_normal::H3IndexBufferManager::getInstance().deal_hexindex_sensor(sensorulid, sensingmediaid, newitem, E_DISPLAY_MODE_WAVE, true);
		}
		///////////////////////////////////////////////////////////////////////////////
	}
}
