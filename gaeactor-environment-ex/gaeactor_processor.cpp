...
HexIdexInfo::HexIdexInfo()
    :m_pH3IndexBufferManager(nullptr),
    m_h3Index(H3_INIT),
...
HexIdexInfo::HexIdexInfo(const HexIdexInfo &other)
{
    ...
    this->m_pH3IndexBufferManager = other.m_pH3IndexBufferManager;
}
...
void HexIdexInfo::init(const H3INDEX& h3, H3CellInfo &&h3cellinfo, H3IndexBufferManager *newPH3IndexBufferManager)
{
    m_h3Index = h3;
    m_h3CellInfo = std::move(h3cellinfo);    
    m_pH3IndexBufferManager = newPH3IndexBufferManager;
}
...
bool HexIdexInfo::updateData(const ENTITY_KEY &entity_key, const H3INDEX& h3,const FLOAT64 &hgt, const UINT8& basecell, const UINT8& resolution, const UINT64& digit_origin, E_DISPLAY_MODE eDdisplayMode, bool bRemove, bool bUpdateIntersect)
{
    ...
    case E_DISPLAY_MODE_ENTITY:
    {
        ...
        if(bUpdateIntersect)
        {
            //            if((old_eHexidexStatus != gaeactorenvironment_ex::E_HEXINDEX_STATUS_ALL && m_eHexidexStatus == gaeactorenvironment_ex::E_HEXINDEX_STATUS_ALL) ||
            //                (old_eHexidexStatus == gaeactorenvironment_ex::E_HEXINDEX_STATUS_ALL && m_eHexidexStatus != gaeactorenvironment_ex::E_HEXINDEX_STATUS_ALL))
            {
                updateIntersect();
            }
        }
...
bool HexIdexInfo::updateData(const SENSOR_KEY &sensor_key, const transdata_param_seq_hexidx &h3, E_DISPLAY_MODE eDdisplayMode, bool bRemove)
{
    ...
    case E_DISPLAY_MODE_WAVE:
    {
        ...
        if(m_sensorlist.size() > 1)
        {
            updateSensorOverlap(m_h3CellInfo.resolution_basecell_digit_valid, m_sensorlist.size());
        }
    }
    break;
    default:
...
void HexIdexInfo::updateIntersect()
{
    ...
    if(m_eHexidexStatus == gaeactorenvironment_ex::E_HEXINDEX_STATUS_ALL)
    {
...
void HexIdexInfo::updateSensorOverlap(const H3INDEX& _h3Index,const UINT32& isize)
{
    if(m_pH3IndexBufferManager && m_pH3IndexBufferManager->m_brecord_sensor_overlap)
    {
        m_pH3IndexBufferManager->updateSensorOverlap(_h3Index,isize);
    }
}
...
H3IndexBufferManager::H3IndexBufferManager()
    ...
    ,m_Cellbuffer(new CELLBUFFER_CONCURRENT_HASHMAP)
    ,m_brecord_sensor_overlap(false)
{
...
H3IndexBufferManager::~H3IndexBufferManager()
{
    if(m_Cellbuffer)
    {
        ...
        ss1<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" H3IndexBufferManager release begin count "<<m_Cellbuffer->size()<<"\n";
        //    TRACE_LOG_PRINT_EX2(ss);
        std::cout<<ss1.str()<<std::endl;
        m_Cellbuffer->clear();
        std::stringstream ss2;
        ss2<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" H3IndexBufferManager release end count "<<m_Cellbuffer->size()<<"\n";
        //    TRACE_LOG_PRINT_EX2(ss);
...
void H3IndexBufferManager::trigger_refresh_event_by_entity_update(const TYPE_ULID& entityid, const tbb::concurrent_hash_map<UINT8, std::tuple<H3INDEX, H3INDEX> > &_hexidx_array_need_replace, IDENTIFI_EVENT_INFO &eventinfo)
{
    ...
    {
        m_pGaeactorProcessor->trigger_refresh_event_by_entity_update(entityid, _hexidx_array_need_replace, eventinfo);
    }
}
...
void H3IndexBufferManager::trigger_refresh_event_by_sensor(const TYPE_ULID &sensorulid, const TYPE_ULID &sensingmediaid, const transdata_sensorposinfo &_sensorinfo, const bool &bNeedClear, const HEXIDX_HGT_ARRAY &remove_hexidxslist, const HEXIDX_HGT_ARRAY &reserve_hexidxslist, const HEXIDX_HGT_ARRAY &append_hexidxslist, IDENTIFI_EVENT_INFO &eventinfo)
{
    if(m_pGaeactorProcessor)
    {
        m_pGaeactorProcessor->trigger_refresh_event_by_sensor(sensorulid, sensingmediaid, _sensorinfo, bNeedClear, remove_hexidxslist, reserve_hexidxslist, append_hexidxslist, eventinfo);
    }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//#define H3_RESOLUTION_BASECELL_DIGIT_OFFSET (H3_NUM_BITS - H3_RES_OFFSET)
#define H3_RESOLUTION_BASECELL_DIGIT_OFFSET (H3_NUM_BITS - H3_RESERVED_OFFSET)
...
#if 0
    UINT64 resolution_basecell_digit_origin = (UINT64)(h3 << H3_RESOLUTION_OFFSET);
...
#else
    UINT64 resolution_basecell_digit_origin = (UINT64)(h3 << H3_RESOLUTION_BASECELL_DIGIT_OFFSET);
    cellinfo.resolution_basecell_digit_valid = resolution_basecell_digit_origin >> H3_RESOLUTION_BASECELL_DIGIT_OFFSET;
#endif
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
typedef std::tuple<H3INDEX, bool, uint32_t> HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE;

#define USING_GET_CELLBUFFER_INFO_CONCURRENT

#ifdef USING_GET_CELLBUFFER_INFO_CONCURRENT
DECLARE_TYPEDEF_TBB_HASH_MAP(UINT64, HEXINDEX_SENSORS_ENTITYS_INFO, HEXINDEX_SENSORS_ENTITYS_INFO_CONCURRENT_HASHMAP)
DECLARE_TYPEDEF_TBB_HASH_MAP(UINT64, HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE, HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE_CONCURRENT_HASHMAP)
#endif

std::vector<HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE> H3IndexBufferManager::getCellbuffers()
{
    std::vector<HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE> ret;
    std::vector<UINT64> keys;
    get_keys(*m_Cellbuffer, keys);
#ifndef USING_GET_CELLBUFFER_INFO_CONCURRENT
    ...
    for(int i = 0; i < keys.size(); i++)
    {
        ...
        if (this->m_Cellbuffer->find(_Cellbuffer_accessor, key))
        {
            ...
            auto val = std::make_tuple(_HexIdexInfo.m_h3Index, _HexIdexInfo.m_bValid.load(), (uint32_t)_HexIdexInfo.m_eHexidexStatus.load());
            ret.push_back(std::move(val));
        }
    }
#else

    HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE_CONCURRENT_HASHMAP ret_tmp;
    tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()),
                      [&](const tbb::blocked_range<size_t>& r) {
                          for (UINT32 i = r.begin(); i != r.end(); ++i)
                          {
                              ...
                              const HexIdexInfo *_HexIdexInfo = nullptr;
                              {
                                  CELLBUFFER_CONCURRENT_HASHMAP_ConstAccessor _Cellbuffer_accessor;
                                  if (this->m_Cellbuffer->find(_Cellbuffer_accessor, key))
                                  {
                                      _HexIdexInfo =  &_Cellbuffer_accessor->second;
                                  }
                              }
                              if(_HexIdexInfo)
                              {
                                  ...
                                  auto val = std::make_tuple(_HexIdexInfo->m_h3Index, _HexIdexInfo->m_bValid.load(), (uint32_t)_HexIdexInfo->m_eHexidexStatus.load());
                                  {
                                      HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE_CONCURRENT_HASHMAP_Accessor _ret_tmp_accessor;
                                      if(!ret_tmp.find(_ret_tmp_accessor, key))
                                      {
                                          ret_tmp.insert(_ret_tmp_accessor,key);
                                          _ret_tmp_accessor->second = std::move(val);
                                      }
                                  }
                              }
                          }
    ...
    ret.reserve(ret_tmp.size());
    {
        HEXINDEX_SENSORS_ENTITYS_INFO_SIMPLE_CONCURRENT_HASHMAP_ConstIterator _ret_tmp_itor = ret_tmp.begin();
        while (_ret_tmp_itor != ret_tmp.end())
        {
            ret.push_back(std::move(_ret_tmp_itor->second));
            _ret_tmp_itor++;
        }
    }
#endif
...
std::vector<HEXINDEX_SENSORS_ENTITYS_INFO>  H3IndexBufferManager::getCellbuffersInfo()
{
    EASY_FUNCTION(profiler::colors::Green)
    std::vector<HEXINDEX_SENSORS_ENTITYS_INFO>   ret;
    std::vector<UINT64> keys;
    get_keys(*m_Cellbuffer, keys);
    ...
    for(int i = 0; i < keys.size(); i++)
    {
        ...
        if (this->m_Cellbuffer->find(_Cellbuffer_accessor, key))
        {
            ...
            const HexIdexInfo &_HexIdexInfo =  _Cellbuffer_accessor->second;
            HEXINDEX_SENSORS_ENTITYS_INFO val = std::make_tuple(_HexIdexInfo.m_h3Index, _HexIdexInfo.m_bValid.load(), (uint32_t)_HexIdexInfo.m_eHexidexStatus.load(), std::move(sensorlist_keys), std::move(entitylist_keys));
            ret.push_back(std::move(val));
        }
    }
#else

    HEXINDEX_SENSORS_ENTITYS_INFO_CONCURRENT_HASHMAP ret_tmp;
    tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()),
                      [&](const tbb::blocked_range<size_t>& r) {
                          for (UINT32 i = r.begin(); i != r.end(); ++i)
                          {
                              ...
                              if(_HexIdexInfo)
                              {
                                  ...
                                  {
                                      HEXINDEX_SENSORS_ENTITYS_INFO_CONCURRENT_HASHMAP_Accessor _ret_tmp_accessor;
                                      if(!ret_tmp.find(_ret_tmp_accessor, key))
                                      {
                                          ret_tmp.insert(_ret_tmp_accessor,key);
                                          _ret_tmp_accessor->second = std::move(val);
    ...
    ret.reserve(ret_tmp.size());
    {
        HEXINDEX_SENSORS_ENTITYS_INFO_CONCURRENT_HASHMAP_ConstIterator _ret_tmp_itor = ret_tmp.begin();
        while (_ret_tmp_itor != ret_tmp.end())
        {
            ret.push_back(std::move(_ret_tmp_itor->second));
            _ret_tmp_itor++;
...
std::vector<HEXINDEX_SENSORS_ENTITYS_INFO> H3IndexBufferManager::getCellbuffersSensorInfo(bool bAll)
{
    EASY_FUNCTION(profiler::colors::Green)
    std::vector<HEXINDEX_SENSORS_ENTITYS_INFO>   ret;
    std::vector<UINT64> keys;
    ...
    {
        get_keys(m_sensor_overlapMap, keys);
    }
#ifndef USING_GET_CELLBUFFER_INFO_CONCURRENT
    ret.reserve(keys.size());
    for(int i = 0; i < keys.size(); i++)
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()),
                      [&](const tbb::blocked_range<size_t>& r) {
                          for (UINT32 i = r.begin(); i != r.end(); ++i)
                          {
                              ...
                              if(_HexIdexInfo && !sensorlist_keys.empty())
                              {
                                  std::vector<std::tuple<TYPE_ULID,FLOAT64>> entitylist_keys;
                                  HEXINDEX_SENSORS_ENTITYS_INFO val = std::make_tuple(_HexIdexInfo->m_h3Index, _HexIdexInfo->m_bValid.load(), (uint32_t)_HexIdexInfo->m_eHexidexStatus.load(), std::move(sensorlist_keys), std::move(entitylist_keys));
...
std::vector<HEXINDEX_SENSORS_ENTITYS_INFO> H3IndexBufferManager::getCellbuffersTargetSensorInfo(const HEXIDX_ARRAY &hexidxslist, bool bAll)
{
    EASY_FUNCTION(profiler::colors::Green)
    std::vector<HEXINDEX_SENSORS_ENTITYS_INFO>   ret;

#ifndef USING_GET_CELLBUFFER_INFO_CONCURRENT
    ret.reserve(hexidxslist.size());
    for(int i = 0; i < hexidxslist.size(); i++)
    {
        auto _h3index = hexidxslist.at(i);
        UINT64 resolution_basecell_digit_origin = (UINT64)(_h3index << H3_RESOLUTION_BASECELL_DIGIT_OFFSET);
        UINT64 key = resolution_basecell_digit_origin >> H3_RESOLUTION_BASECELL_DIGIT_OFFSET;
        CELLBUFFER_CONCURRENT_HASHMAP_ConstAccessor _Cellbuffer_accessor;
        if (this->m_Cellbuffer->find(_Cellbuffer_accessor, key))
        {
            std::vector<std::tuple<TYPE_ULID, TYPE_ULID,transdata_param_seq_hexidx_hgt>> sensorlist_keys;
                          ...
                          for (size_t i = r.begin(); i != r.end(); ++i)
                          {
                              ...
                              if (E_SUCCESS == cellToParent(h3, res, &parentindex))
                              {
                                  ...
                                  bool bExist = false;
                                  {
                                      ...
                                      if(this->m_Cellbuffer->find(_Cellbuffer_accessor, h3cellinfo.resolution_basecell_digit_valid))
                                      {
                                          bExist = true;
                                          ...
                                          {
                                              this->m_Cellbuffer->erase(_Cellbuffer_accessor);
                                          }
                                      }
                                  }
                                  if(!bExist && !bRemove)
                                  {
                                      ...
                                      _hexindexinfo.init(parentindex, std::move(h3cellinfo),this);

                                      {
                                          CELLBUFFER_CONCURRENT_HASHMAP_Accessor _Cellbuffer_accessor;
                                          this->m_Cellbuffer->insert(_Cellbuffer_accessor, h3cellinfo.resolution_basecell_digit_valid);
    ...
    auto update_hexidx_info=[&](const H3INDEX &h3, const H3INDEX &parentindex, bool bRemove, bool bUpdateIntersect = true){
        ...
        bool bExist = false;
        {
            ...
            if(this->m_Cellbuffer->find(_Cellbuffer_accessor, h3cellinfo.resolution_basecell_digit_valid))
            {
                ...
                {
                    this->m_Cellbuffer->erase(_Cellbuffer_accessor);
//                    std::cout<<" rem hex------------------ "<<" "<<std::hex<<h3<<" "<<std::dec<<resolution<<" "<<std::hex<<parentindex<<" "<<std::dec<<H3_GET_RESOLUTION(parentindex)<<" "<<this->m_Cellbuffer->size()<<" \n";
                }
            }
        }
        if(!bExist && !bRemove)
...
//            std::cout<<" add hex++++++++++++++++++ "<<" "<<std::hex<<h3<<" "<<std::dec<<resolution<<" "<<std::hex<<parentindex<<" "<<std::dec<<H3_GET_RESOLUTION(parentindex)<<" "<<this->m_Cellbuffer->size()<<" \n";
        }
    };
            ...
            if(old_hexidx != 0 || new_hexidx != 0)
            {
                ...
                case E_EVENT_MODE_UPDATE:
                {
...

                    {
                        ...
                        if(this->m_Cellbuffer->find(_Cellbuffer_constaccessor, new_h3cellinfo.resolution_basecell_digit_valid))
                        {
                            if(_Cellbuffer_constaccessor->second.m_eHexidexStatus == E_HEXINDEX_STATUS_ALL)
                            {
                                ...
                                {
                                    m_pH3IndexBufferManager->lockIntersectForWrite();
                                }
                                m_pH3IndexBufferManager->replaceIntersect(entity_key, old_hexidx, new_hexidx, std::move(interscet));
                                if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
                                {
                                    m_pH3IndexBufferManager->unlockIntersect();
                                }
                            }
                            else
                            {
                                m_pH3IndexBufferManager->removeIntersect_ex(entity_key,old_hexidx);
                            }
                        }
...

                                          {
                                              ...
                                              if(this->m_Cellbuffer->find(_Cellbuffer_constaccessor, new_h3cellinfo.resolution_basecell_digit_valid))
                                              {
                                                  if(_Cellbuffer_constaccessor->second.m_eHexidexStatus == E_HEXINDEX_STATUS_ALL)
                                                  {
                                                      ...
                                                      if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
                                                      {
                                                          this->lockIntersectForWrite();
                                                      }
                                                      this->replaceIntersect(entity_key, old_hexidx, new_hexidx, std::move(interscet));
                                                      if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
                                                      {
                                                          this->unlockIntersect();
                                                      }
                                                  }
                                                  else
                                                  {
                                                      this->removeIntersect_ex(entity_key,old_hexidx);
                                                  }
                                              }
                                          }
                                      }
...
void H3IndexBufferManager::deal_hexindex_sensor(const SENSOR_KEY &sensor_key, const transdata_param_seq_hexidx& h3, bool bRemove)
{
    ...
    bool bExist = false;
    {
        ...
        if(this->m_Cellbuffer->find(_Cellbuffer_accessor, h3cellinfo.resolution_basecell_digit_valid))
        {
            ...
            {
                this->m_Cellbuffer->erase(_Cellbuffer_accessor);
                //            std::cout<<" rem hex------------------ "<<" "<<std::hex<<h3<<" "<<std::dec<<std::dec<<H3_GET_RESOLUTION(h3)<<" "<<this->m_Cellbuffer->size()<<" \n";
            }
        }
    }
    ...
    {
        HexIdexInfo _hexindexinfo;
        _hexindexinfo.init(h3.PARAM_seq_hexidx_element, std::move(h3cellinfo), this);
        {
            CELLBUFFER_CONCURRENT_HASHMAP_Accessor _Cellbuffer_accessor;
            this->m_Cellbuffer->insert(_Cellbuffer_accessor, h3cellinfo.resolution_basecell_digit_valid);
            _Cellbuffer_accessor->second = std::move(_hexindexinfo);
        ...
        //                std::cout<<" add hex++++++++++++++++++ "<<" "<<std::hex<<h3<<" "<<std::dec<<H3_GET_RESOLUTION(h3)<<" "<<this->m_Cellbuffer->size()<<" \n";
    }
}
...
void H3IndexBufferManager::reset()
{
...
#if 0
    {
        ...
        tbb::blocked_range<CELLBUFFER_CONCURRENT_HASHMAP_Iterator> range1(this->m_Cellbuffer->begin(), this->m_Cellbuffer->end());
        tbb::parallel_for_each(range1, [&](const tbb::blocked_range<CELLBUFFER_CONCURRENT_HASHMAP_Iterator> &range1) {

            for (auto &data:range1)
            {
                SENESOR_LIST_CONCURRENT_HASHMAP sensorlist = data.second.sensorlist();
                tbb::blocked_range<SENESOR_LIST_CONCURRENT_HASHMAP_Iterator> rangesub(sensorlist.begin(), sensorlist.end());
                tbb::parallel_for_each(rangesub, [&](const tbb::blocked_range<SENESOR_LIST_CONCURRENT_HASHMAP_Iterator> &rangesub) {

                    for (auto &subdata:rangesub)
                    {
                        const SENSOR_KEY &sensor_key = subdata.first;
                        transdata_param_seq_hexidx _transdata_param_seq_hexidx{data.second.m_h3Index, transdata_param_seq_hexidx_hgt{0,0,0,0}};
                        bool bvalid = data.second.updateData(sensor_key, _transdata_param_seq_hexidx, E_DISPLAY_MODE_WAVE, true);
                        if(!bvalid &&
                            !data.second.m_bValid&&
                            data.second.m_eHexidexStatus == E_HEXINDEX_STATUS_FREE)
                        {
                            this->m_Cellbuffer->erase(data.first);
                            //            std::cout<<" rem hex------------------ "<<" "<<std::hex<<h3<<" "<<std::dec<<std::dec<<H3_GET_RESOLUTION(h3)<<" "<<this->m_Cellbuffer->size()<<" \n";
                        }
                    }
                });
                      ...
                      [&](const tbb::blocked_range<size_t>& r) {
                          for (size_t i = r.begin(); i != r.end(); ++i)
                          {
                              ...
                              if (this->m_Cellbuffer->find(_Cellbuffer_accessor, key))
                              {
                                  ...
                                  //                transdata_param_seq_hexidx _transdata_param_seq_hexidx{_Cellbuffer_accessor->second.m_h3Index, transdata_param_seq_hexidx_hgt{0,0,0,0}};
...
//    this->m_Cellbuffer->clear();
    if(this->m_Cellbuffer)
    {
        delete m_Cellbuffer;
        m_Cellbuffer = new CELLBUFFER_CONCURRENT_HASHMAP;
    }
    m_sensor_overlapMap.clear();
    m_IntersectInfoMap.clear();
}

void H3IndexBufferManager::record_sensor_overlap(bool brecord)
{
    m_brecord_sensor_overlap = brecord;
}

void H3IndexBufferManager::updateSensorOverlap(const H3INDEX &_h3Index, const UINT32 &isize)
{
    SENSOR_OVERLAP_CONCURRENT_HASHMAP_Accessor _sensor_overlapMap_accessor;
    if(m_sensor_overlapMap.find(_sensor_overlapMap_accessor, _h3Index))
    {
        _sensor_overlapMap_accessor->second = isize;
    }
    else
    {
        m_sensor_overlapMap.insert(_sensor_overlapMap_accessor, _h3Index);
        _sensor_overlapMap_accessor->second = isize;
    }
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
GaeactorProcessor::GaeactorProcessor()
    :m_pH3IndexBufferManager(nullptr)
    ,m_entityInfosMap(nullptr)
    ,m_sensorInfosMap(nullptr)
    ,m_exist_events_map(nullptr)
    ,m_entity_events_map(nullptr)
    ,m_sensor_events_map(nullptr)
    ,m_SensorInfoManager(nullptr)
    ,m_EntityInfoManager(nullptr)
{
    init();
    //    // 设置线程池大小为 4
...
{
    std::stringstream ss1;
    ss1<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor release begin\n";
    //    TRACE_LOG_PRINT_EX2(ss1);
    std::cout<<ss1.str()<<std::endl;

    reset();

    std::stringstream ss;
    ss<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor release end\n";
    //    TRACE_LOG_PRINT_EX2(ss);
...
void GaeactorProcessor::init()
{
    DELETE_PTR(m_entityInfosMap)
    DELETE_PTR(m_sensorInfosMap)
    DELETE_PTR(m_exist_events_map)
    DELETE_PTR(m_entity_events_map)
    DELETE_PTR(m_sensor_events_map)
    DELETE_PTR(m_SensorInfoManager)
    DELETE_PTR(m_EntityInfoManager)
    DELETE_PTR(m_pH3IndexBufferManager)

    if(nullptr == m_pH3IndexBufferManager)
    {
        m_pH3IndexBufferManager = new H3IndexBufferManager();
        m_pH3IndexBufferManager->setGaeactorProcessor(this);
    }
    if(nullptr == m_entityInfosMap)
    {
        m_entityInfosMap = new ENTITYINFOS_CONCURRENT_HASHMAP();
    }
    if(nullptr == m_sensorInfosMap)
    {
        m_sensorInfosMap = new SENSORINFOS_CONCURRENT_HASHMAP();
    }
    if(nullptr == m_exist_events_map)
    {
        m_exist_events_map = new EXIST_EVENTS_CONCURRENT_HASHMAP();
    }
    if(nullptr == m_entity_events_map)
    {
        m_entity_events_map = new ENTITY_EVENTS_CONCURRENT_HASHMAP();
    }
    if(nullptr == m_sensor_events_map)
    {
        m_sensor_events_map = new SENSOR_EVENTS_CONCURRENT_HASHMAP();
    }
    if(nullptr == m_SensorInfoManager)
    {
        m_SensorInfoManager = new SensorInfoManager();
    }
    if(nullptr == m_EntityInfoManager)
    {
        m_EntityInfoManager = new EntityInfoManager();
    }
}
...
void GaeactorProcessor::clearEntityInfo(const TYPE_ULID &entityid)
{
    ...
    if(this->m_entityInfosMap->find(_entityInfosMap_accessor, entityid))
    {
        this->m_EntityInfoManager->release_entityinfo_element(_entityInfosMap_accessor->second.m_pEntitySensorInfoElement);
        this->m_entityInfosMap->erase(_entityInfosMap_accessor);
    }
}

H3IndexBufferManager& GaeactorProcessor::getBuffer()
{
    return *m_pH3IndexBufferManager;
}
                                               ...
                                               IDENTIFI_EVENT_INFO& eventinfo)
{
    ...
    {
        ENTITYINFOS_CONCURRENT_HASHMAP_Accessor _entityInfosMap_accessor;
        if(this->m_entityInfosMap->find(_entityInfosMap_accessor, uildsrc))
        {
        ...
        else
        {
            if(!bNeedClear)
            {
                ...
                tagEntityPosInfo _tagEntityPosInfo{_entityinfo, (PROPERTY_GET_TYPE(_entityinfo.PARAM_sensor_property) == AGENT_ENTITY_PROPERTY_SENSOR)? true : false};
                hexindexentityinfo.m_pEntitySensorInfoElement = this->m_EntityInfoManager->alloc_entityinfo_element(std::move(_tagEntityPosInfo));
                _pEntitySensorInfoElement = hexindexentityinfo.m_pEntitySensorInfoElement;
                this->m_entityInfosMap->insert(_entityInfosMap_accessor, uildsrc);
    ...
    if(bDeal)
    {
        m_pH3IndexBufferManager->deal_hexindex_entity_remove_old_append_new(ENTITY_KEY{uildsrc, _pEntitySensorInfoElement}, oldh3, newh3, hgt, _hexidx_array_need_replace);
    }
    else
    {
    ...
    if(bUpdateEvent)
    {
        ...
        {
            m_pH3IndexBufferManager->trigger_refresh_event_by_entity_update(uildsrc, _hexidx_array_need_replace, eventinfo);
        }
    }
...
#endif
//    std::cout<<"+++++++++++++++++++++++++++m_entityInfosMap size "<<this->m_entityInfosMap->size()<<"\n";
                                               ...
                                               IDENTIFI_EVENT_INFO &eventinfo)
{
    ...
    auto appendnewitem=[&](tagHexindexSensorInfo *hexindexsensorinfo ){
        ...
        hexindexlistensorinfo.setPSensorInfoManager(m_SensorInfoManager);
        if (!bNeedClear)
        {
            hexindexlistensorinfo.m_pSensorInfoElement = this->m_SensorInfoManager->alloc_sensorinfo_element(std::move(_sensorinfo));
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, _hexidxs.size()),
                          [&](const tbb::blocked_range<size_t>& r) {
                              for (size_t i = r.begin(); i != r.end(); ++i)
                              {
                                  ...
                                  m_pH3IndexBufferManager->deal_hexindex_sensor(SENSOR_KEY{hexindexsensorinfo->m_sensorulid, sensingmediaid, hexindexlistensorinfo.m_pSensorInfoElement}, _hexidxs_item, false);
                              }
                          });
    ...
    {
        {
            ...
            };
            {
                ...
                if(this->m_sensorInfosMap->find(_sensorInfosMap_accessor, sensorulid))
                {
...
#endif
        {
            tagHexindexSensorInfo *hexindexsensorinfo = new tagHexindexSensorInfo(m_pH3IndexBufferManager);
            hexindexsensorinfo->m_bValid = true;
            ...
            {
                SENSORINFOS_CONCURRENT_HASHMAP_Accessor _sensorInfosMap_accessor;
                this->m_sensorInfosMap->insert(_sensorInfosMap_accessor, sensorulid);
    ...
    if(bUpdateEvent)
    {
        if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_GENERATE_DETECT_EVENT_AFTER_AGENT_STEP_UPDATE_SENSOR()  /*&& !runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE()*/)
        {
            if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
            {
                ...
                {
                    m_pH3IndexBufferManager->trigger_refresh_event_by_sensor(sensorulid,
                                                                            sensingmediaid,
                                                                            tmp_sensorinfo,
                                                                            bNeedClear,
                                                                            remove_hexidxslist,
...
//    stdutils::OriDateTime::sleep(20);
    //此处应该是需要休眠，但只有输出才有效？
    //    std::cout<<"+++++++++++++++++++++++++++m_sensorInfosMap size "<<this->m_sensorInfosMap->size()<<"\n";
}

void GaeactorProcessor::reset()
{
    std::stringstream ss;
    ss<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor reset begin 1\n";
//    TRACE_LOG_PRINT_EX2(ss);
    std::cout<<ss.str()<<std::endl;

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
//        tbb::blocked_range<ENTITYINFOS_CONCURRENT_HASHMAP_Iterator> range1(this->m_entityInfosMap->begin(), this->m_entityInfosMap->end());
//        tbb::parallel_for_each(range1, [&](const tbb::blocked_range<ENTITYINFOS_CONCURRENT_HASHMAP_Iterator> &range1) {

//            for (auto &data:range1)
//            {
//                if(data.second.m_pEntitySensorInfoElement)
//                {
//                    this->m_EntityInfoManagersMap->release_entityinfo_element(data.second.m_pEntitySensorInfoElement);
//                }
//            }
//        });

        std::vector<UINT64> keys;
        get_keys(*m_entityInfosMap, keys);
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()),
                          [&](const tbb::blocked_range<size_t>& r) {
                              ...
                              {
                                  const auto &key = keys[i];
                                  ENTITYINFOS_CONCURRENT_HASHMAP_ConstAccessor _accessor;
                                  if(m_entityInfosMap->find(_accessor, key))
                                  {
                                      if(_accessor->second.m_pEntitySensorInfoElement)
                                      {
                                          m_EntityInfoManager->release_entityinfo_element(_accessor->second.m_pEntitySensorInfoElement);
                                      }
                                  }
                              }
                          });

//        ENTITYINFOS_CONCURRENT_HASHMAP_ConstIterator _entityInfosMap_itor = this->m_entityInfosMap->begin();
//        while(_entityInfosMap_itor != this->m_entityInfosMap->end())
//        {
//            if(_entityInfosMap_itor->second.m_pEntitySensorInfoElement)
//            {
//                this->m_EntityInfoManagersMap->release_entityinfo_element(_entityInfosMap_itor->second.m_pEntitySensorInfoElement);
//            }
//        }

//        this->m_entityInfosMap->clear();

        DELETE_PTR(m_entityInfosMap)
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    std::stringstream ss2;
    ss2<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor reset begin 2\n";
    //    TRACE_LOG_PRINT_EX2(ss);
    std::cout<<ss2.str()<<std::endl;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    {
//        tbb::blocked_range<SENSORINFOS_CONCURRENT_HASHMAP_Iterator> range1(this->m_sensorInfosMap->begin(), this->m_sensorInfosMap->end());
//        tbb::parallel_for_each(range1, [&](const tbb::blocked_range<SENSORINFOS_CONCURRENT_HASHMAP_Iterator> &range1) {
...
//        });
        ...
        std::vector<UINT64> keys;
        get_keys(*m_sensorInfosMap, keys);
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, keys.size()),
                          [&](const tbb::blocked_range<size_t>& r) {
                              ...
                              {
                                  const auto &key = keys[i];
                                  SENSORINFOS_CONCURRENT_HASHMAP_ConstAccessor _accessor;
                                  if(m_sensorInfosMap->find(_accessor, key))
                                  {
                                      if(_accessor->second)
                                      {
                                          _accessor->second->setPH3IndexBufferManager(nullptr);
                                          delete _accessor->second;
                                      }
                                  }
                              }
                          });

//        SENSORINFOS_CONCURRENT_HASHMAP_ConstIterator _sensorInfosMap_itor = this->m_sensorInfosMap->begin();
//        while(_sensorInfosMap_itor != this->m_sensorInfosMap->end())
//        {
//            if(_sensorInfosMap_itor->second)
//            {
//                delete _sensorInfosMap_itor->second;
//            }
...
//        this->m_sensorInfosMap->clear();
    ...
    ss3<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor reset begin 3\n";
    //    TRACE_LOG_PRINT_EX2(ss);
    std::cout<<ss3.str()<<std::endl;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//    this->m_exist_events_map->clear();
//    this->m_entity_events_map->clear();
//    this->m_sensor_events_map->clear();
    ...
    ss4<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor reset begin 4\n";
    //    TRACE_LOG_PRINT_EX2(ss);
    std::cout<<ss4.str()<<std::endl;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//    this->m_SensorInfoManager->reset();
//    this->m_EntityInfoManagersMap->reset();


    DELETE_PTR(m_SensorInfoManager)
    DELETE_PTR(m_EntityInfoManager)
    std::stringstream ss5;
    ss5<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor reset begin 5\n";
//    TRACE_LOG_PRINT_EX2(ss1);
    std::cout<<ss5.str()<<std::endl;

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //    m_pH3IndexBufferManager->reset();

    DELETE_PTR(m_pH3IndexBufferManager)
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    std::stringstream ss1;
    ss1<<stdutils::OriDateTime::currentDateTimeTimestamptoString()<<" "<<SettingsConfig::getInstance().getCommserviceName()<<" GaeactorProcessor reset end \n";
    //    TRACE_LOG_PRINT_EX2(ss);
    std::cout<<ss1.str()<<std::endl;
}
...
void GaeactorProcessor::refresh_silent_timeout()
{
    ...
    PARALLEL_BEGIN
        SENSORINFOS_CONCURRENT_HASHMAP_Accessor _sensorInfosMap_accessor;
    if (this->m_sensorInfosMap->find(_sensorInfosMap_accessor, key))
    {
        tagHexindexSensorInfo* ptagHexindexSensorInfo =  _sensorInfosMap_accessor->second;
        if(ptagHexindexSensorInfo)
        {
...
transdata_entityposinfo GaeactorProcessor::gettransdata_entityposinfo(const TYPE_ULID &sensorulid)
{
    ...
    if(this->m_entityInfosMap->find(_entityInfosMap_constaccessor, sensorulid))
    {
...
bool GaeactorProcessor::isEntityHaveSensorProperty(const TYPE_ULID &entityulid)
{
    ...
    ENTITYINFOS_CONCURRENT_HASHMAP_ConstAccessor _entityInfosMap_constaccessor;
    if(this->m_entityInfosMap->find(_entityInfosMap_constaccessor, entityulid))
    {
        if(_entityInfosMap_constaccessor->second.m_pEntitySensorInfoElement)
        {
...
void GaeactorProcessor::trigger_refresh_event_by_entity_update(const TYPE_ULID &entityid, const tbb::concurrent_hash_map<UINT8, std::tuple<H3INDEX, H3INDEX> > &_hexidx_array_need_replace, IDENTIFI_EVENT_INFO &eventinfo)
{
    ...
    {
        {
            ...
            if(this->m_entity_events_map->find(_entity_events_map_accessor, key.entityid))
            {
...

        {
            ...
            if(this->m_sensor_events_map->find(_sensor_events_map_accessor, _SENSOR_KEY))
            {
                EVENTS_VALID_CONCURRENT_HASHMAP_Accessor _EVENTS_VALID_CONCURRENT_HASHMAP_tmp_accessor;
    ...
    {
        {
            ...
            else
            {
                ...
                this->m_entity_events_map->insert(_entity_events_map_accessor, key.entityid);
...

        {
            ...
            else
            {
                EVENTS_VALID_CONCURRENT_HASHMAP _EVENTS_VALID_CONCURRENT_HASHMAP;
                _EVENTS_VALID_CONCURRENT_HASHMAP.insert(std::make_pair(key, true));
                this->m_sensor_events_map->insert(_sensor_events_map_accessor, _SENSOR_KEY);
    ...
    };
    {
        ...
        EVENTS_VALID_CONCURRENT_HASHMAP _EVENTS_VALID_CONCURRENT_HASHMAP;
        {
            ...
            if(this->m_entity_events_map->find(_entity_events_map_constaccessor, entityid))
            {
        ...
        auto getintersectInfos=[&](const H3INDEX h3index)
        {
            if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
            {
                m_pH3IndexBufferManager->lockIntersectForRead();
            }
            INTERSECTINFO_CONCURRENT_HASHMAP_ConstAccessor _exist_events_map_constaccessor;
            if (m_pH3IndexBufferManager->m_IntersectInfoMap.find(_exist_events_map_constaccessor, h3index))
            {
        ...
        for(auto _intersectInfos_itor = intersectInfos.begin();_intersectInfos_itor != intersectInfos.end(); _intersectInfos_itor++)
        {
            ...
            for(auto sensor_list_itor = sensorlist.begin(); sensor_list_itor != sensorlist.end(); sensor_list_itor++)
            {
                ...
                const gaeactorenvironment_ex::BufferInfoElement<transdata_sensorposinfo>*_sensorinfo_ptr = sensorlist_key._sensorinfo_ptr;
                ...
                for(auto entitylist_itor = entitylist.begin(); entitylist_itor != entitylist.end(); entitylist_itor++)
                {
                    ...
                    const gaeactorenvironment_ex::BufferInfoElement<tagEntityPosInfo>* _entityinfo_ptr = entitylist_key._entityinfo_ptr;
                    ...
                    {
                        EXIST_EVENTS_CONCURRENT_HASHMAP_Accessor _exist_events_map_accessor;
                        if (this->m_exist_events_map->find(_exist_events_map_accessor, eventtile))
                        {
...
//                            }
                            ...
                            this->m_exist_events_map->insert(_exist_events_map_accessor, eventtile);
                                                        ...
                                                        [&](const tbb::blocked_range<size_t>& sensorlist_keys_r) {
                                                            for (size_t sensorlist_keys_i = sensorlist_keys_r.begin(); sensorlist_keys_i != sensorlist_keys_r.end(); ++sensorlist_keys_i)
                                                            {
                                                                ...
                                                                if (_intersectInfos_constaccessor->second.sensorlist.find(_sensorlist_constaccessor, sensorlist_key))
                                                                {
                                                                    ...
                                                                    const gaeactorenvironment_ex::BufferInfoElement<transdata_sensorposinfo>*_sensorinfo_ptr = _sensorlist_constaccessor->first._sensorinfo_ptr;
                                                                                      ...
                                                                                      [&](const tbb::blocked_range<size_t>& entitylist_keys_r) {
                                                                                          for (size_t entitylist_keys_i = entitylist_keys_r.begin(); entitylist_keys_i != entitylist_keys_r.end(); ++entitylist_keys_i)
                                                                                          {
                                                                                              ...
                                                                                              if (_intersectInfos_constaccessor->second.entitylist.find(_entitylist_constaccessor, entitylist_key))
                                                                                              {
                                                                                                  ...
                                                                                                  const gaeactorenvironment_ex::BufferInfoElement<tagEntityPosInfo>*_entityinfo_ptr = _entitylist_constaccessor->first._entityinfo_ptr;
                                                                                                  if (sensorulid == entityulid)
                                                                                                  {
                                                                                                      continue;
                                                                                                  }
                              ...
                              for (size_t i = r.begin(); i != r.end(); ++i)
                              {
                                  ...
                                  if (this->m_exist_events_map->find(_exist_events_map_accessor, key))
                                  {
                                      ...
                                      if (!_exist_events_map_accessor->second.second)
                                      {
                                          ...
                                          this->m_exist_events_map->erase(_exist_events_map_accessor);
...
void GaeactorProcessor::trigger_refresh_event_by_sensor(const TYPE_ULID &sensorid, const TYPE_ULID &sensingmedia_id, const transdata_sensorposinfo &_sensorinfo, const bool &bNeedClear, const HEXIDX_HGT_ARRAY &remove_hexidxslist, const HEXIDX_HGT_ARRAY &reserve_hexidxslist, const HEXIDX_HGT_ARRAY &append_hexidxslist, IDENTIFI_EVENT_INFO &eventinfo)
{
    ...
    };
    {
        ...
        EVENTS_VALID_CONCURRENT_HASHMAP _EVENTS_VALID_CONCURRENT_HASHMAP;
        {
            ...
            if(this->m_sensor_events_map->find(_entity_events_map_constaccessor, _SENSOR_KEY))
            {
                _EVENTS_VALID_CONCURRENT_HASHMAP = _entity_events_map_constaccessor->second;
            }
        }
...
void GaeactorProcessor::refresh_event_type(IDENTIFI_EVENT_INFO& identifi_event_info, const TYPE_ULID &entityid, const TYPE_ULID &sensorid, const TYPE_ULID &sensingmedia_id, int itype)
{
...

    {
...

        {
            ...
            get_keys(*this->m_exist_events_map, _exist_events_map_keys);
        ...
        //    INTERSECTINFO_CONCURRENT_HASHMAP intersectInfos = m_pH3IndexBufferManager->getIntersectInfos();

        INTERSECTINFO_CONCURRENT_HASHMAP intersectInfos;

        std::vector<H3INDEX> intersectInfos_keys;
        get_keys(m_pH3IndexBufferManager->m_IntersectInfoMap, intersectInfos_keys);
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, intersectInfos_keys.size()),
                          [&](const tbb::blocked_range<size_t>& r) {
                              ...
                              {
                                  INTERSECTINFO_CONCURRENT_HASHMAP_ConstAccessor _exist_events_map_constaccessor;
                                  if (m_pH3IndexBufferManager->m_IntersectInfoMap.find(_exist_events_map_constaccessor, intersectInfos_keys[i]))
                                  {
                                      INTERSECTINFO_CONCURRENT_HASHMAP_Accessor intersectInfos_accessor;
                                      intersectInfos.insert(intersectInfos_accessor, _exist_events_map_constaccessor->first);
                                      intersectInfos_accessor->second = _exist_events_map_constaccessor->second;
...
tagHexindexlistSensorInfo::~tagHexindexlistSensorInfo()
{
    m_hexIndexslist.clear();
    m_pSensorInfoManager->release_sensorinfo_element(this->m_pSensorInfoElement);
}
                                                                           ...
                                                                           const UINT64 &currentTimeStamp)
{
    ...
    BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP build_remove_hexidxslist_hexIndexslist_;
    BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP build_reserve_hexidxslist_hexIndexslist_;
    BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP build_append_hexidxslist_hexIndexslist_;
    ...
    if(this->m_hexIndexslist.find(_hexIndexslist_accessor, key))
    {
        ...
        if (_hexIndexslist_accessor->second.m_bHexidxValid)
        {
                ...
                if (newhexidxslist_itor != new_hexidxslist.end())
                {
                    _hexIndexslist_accessor->second.m_bHexidxValid = false;
                    //旧的存在 新的不存在 移除
                    BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP_Accessor build_remove_hexidxslist_hexIndexslist_accessor;
                    build_remove_hexidxslist_hexIndexslist_.insert(build_remove_hexidxslist_hexIndexslist_accessor, _hexIndexslist_accessor->first);
                    build_remove_hexidxslist_hexIndexslist_accessor->second = _hexIndexslist_accessor->second.m_HexidxInfo.PARAM_seq_hexidx_hgt;
                    //清理
                    m_hexIndexslist.erase(_hexIndexslist_accessor);
                }
    //            else
    ...
    //                BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP_Accessor build_reserve_hexidxslist_hexIndexslist_accessor;
    //                build_reserve_hexidxslist_hexIndexslist_.insert(build_reserve_hexidxslist_hexIndexslist_accessor, _hexIndexslist_accessor->first);
    //                build_reserve_hexidxslist_hexIndexslist_accessor->second = _hexIndexslist_accessor->second.m_HexidxInfo.PARAM_seq_hexidx_hgt;
                          ...
                          for (size_t i = r.begin(); i != r.end(); ++i)
                          {
                              ...
                              if (this->m_hexIndexslist.find(_hexIndexslist_accessor, _hexidxs_item.PARAM_seq_hexidx_element))
                              {
                                  if (!_hexIndexslist_accessor->second.m_bHexidxValid)
                                  {
                                      ...
                                      BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP_Accessor build_append_hexidxslist_hexIndexslist_accessor;
                                      build_append_hexidxslist_hexIndexslist_.insert(build_append_hexidxslist_hexIndexslist_accessor, _hexidxs_item.PARAM_seq_hexidx_element);
                                      build_append_hexidxslist_hexIndexslist_accessor->second = _hexidxs_item.PARAM_seq_hexidx_hgt;
                                  }
                                  else
                                  {
                                      //旧的存在 新的存在 保留
                                      ...
                                      build_reserve_hexidxslist_hexIndexslist_.insert(build_reserve_hexidxslist_hexIndexslist_accessor, _hexIndexslist_accessor->first);
//                                      build_reserve_hexidxslist_hexIndexslist_accessor->second = _hexIndexslist_accessor->second.m_HexidxInfo.PARAM_seq_hexidx_hgt;
                                      build_reserve_hexidxslist_hexIndexslist_accessor->second = _hexidxs_item.PARAM_seq_hexidx_hgt;
                                  }
                              }
                              else
                              {
    ...
    {
        reserve_hexidxslist.reserve(build_reserve_hexidxslist_hexIndexslist_.size());
        BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP_ConstIterator _exist_events_map_itor = build_reserve_hexidxslist_hexIndexslist_.begin();
        while (_exist_events_map_itor != build_reserve_hexidxslist_hexIndexslist_.end())
        {
    ...
    {
        remove_hexidxslist.reserve(build_remove_hexidxslist_hexIndexslist_.size());
        BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP_ConstIterator _exist_events_map_itor = build_remove_hexidxslist_hexIndexslist_.begin();
        while (_exist_events_map_itor != build_remove_hexidxslist_hexIndexslist_.end())
        {
    ...
    {
        append_hexidxslist.reserve(build_append_hexidxslist_hexIndexslist_.size());
        BUILD_HEXIDX_TIME_STAMP_INFO_CONCURRENT_HASHMAP_ConstIterator _exist_events_map_itor = build_append_hexidxslist_hexIndexslist_.begin();
        while (_exist_events_map_itor != build_append_hexidxslist_hexIndexslist_.end())
        {
...
void tagHexindexlistSensorInfo::setPSensorInfoManager(SensorInfoManager *newPSensorInfoManager)
{
    m_pSensorInfoManager = newPSensorInfoManager;
}
...
tagHexindexSensorInfo::tagHexindexSensorInfo(H3IndexBufferManager *_pH3IndexBufferManager)
            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, remove_hexidxslist.size()),
                              [&](const tbb::blocked_range<size_t>& r) {
                                  for (size_t i = r.begin(); i != r.end(); ++i)
                                  {
                                      ...
                                      if(m_pH3IndexBufferManager)
                                      {
                                          m_pH3IndexBufferManager->deal_hexindex_sensor(SENSOR_KEY{m_sensorulid, sensingmediaid, psensorInfoele}, h3, true);
                                      }
                                  }
                              });
        }
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, append_hexidxslist.size()),
                          [&](const tbb::blocked_range<size_t>& r) {
                              for (size_t i = r.begin(); i != r.end(); ++i)
                              {
                                  ...
                                  if(m_pH3IndexBufferManager)
                                  {
                                      m_pH3IndexBufferManager->deal_hexindex_sensor(SENSOR_KEY{m_sensorulid, sensingmediaid, psensorInfoele}, h3, false);
                                  }
                              }
                          });
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, hexidxslist.size()),
                      [&](const tbb::blocked_range<size_t>& r) {
                          for (size_t i = r.begin(); i != r.end(); ++i)
                          {
                              if(m_pH3IndexBufferManager)
                              {
                                  m_pH3IndexBufferManager->deal_hexindex_sensor(SENSOR_KEY{m_sensorulid, sensingmediaid, psensorInfoele}, hexidxslist[i], true);
                              }
                          }
                      });
}
...
void tagHexindexSensorInfo::setPH3IndexBufferManager(H3IndexBufferManager *newPH3IndexBufferManager)
{
    m_pH3IndexBufferManager = newPH3IndexBufferManager;
}
...
void SensorInfoManager::reset()
{
    m_sensor_info_buffer_manager.reset();
}
...
void EntityInfoManager::reset()
{
    m_entity_info_buffer_manager.reset();
}
