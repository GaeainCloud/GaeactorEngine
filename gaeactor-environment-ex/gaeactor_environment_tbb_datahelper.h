...
typedef struct sensor_sensingmedia_index_key_type
{
    ...
    gaeactorenvironment_ex::BufferInfoElement<transdata_sensorposinfo>* _sensorinfo_ptr;
}SENSOR_KEY;
...
struct hash<SENSOR_KEY> {
    size_t operator()(const SENSOR_KEY& p)const {
        return std::hash<TYPE_ULID>()(p.sensorid)
               ...
               ^ std::hash<gaeactorenvironment_ex::BufferInfoElement<transdata_sensorposinfo>*>()(p._sensorinfo_ptr);
    }
};
...
{
    TYPE_ULID entityid;
    gaeactorenvironment_ex::BufferInfoElement<tagEntityPosInfo>* _entityinfo_ptr;
}ENTITY_KEY;
...
struct hash<ENTITY_KEY> {
    size_t operator()(const ENTITY_KEY& p)const {
        return std::hash<TYPE_ULID>()(p.entityid)
               ^ std::hash<gaeactorenvironment_ex::BufferInfoElement<tagEntityPosInfo>*>()(p._entityinfo_ptr);
    }
};
