...
{
    m_pGaeactorProcessor_normal = new gaeactorenvironment_ex_normal::GaeactorProcessor();
    m_pGaeactorProcessor = new gaeactorenvironment_ex::GaeactorProcessor();
}
...
void GaeactorProcessorInterfaceInstance::refresh_silent_timeout()
{
    if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
    {
        if(m_pGaeactorProcessor_normal)
        {
            m_pGaeactorProcessor_normal->refresh_silent_timeout();
        }
    }
    else
    {
        if(m_pGaeactorProcessor)
        {
            m_pGaeactorProcessor->refresh_silent_timeout();
        }
    }
}


void GaeactorProcessorInterfaceInstance::refresh_event(IDENTIFI_EVENT_INFO &identifi_event_info)
{
    if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
    {
        if(m_pGaeactorProcessor_normal)
        {
            m_pGaeactorProcessor_normal->refresh_event(identifi_event_info);
        }
    }
    else
    {
        if(m_pGaeactorProcessor)
        {
            m_pGaeactorProcessor->refresh_event(identifi_event_info);
        }
    }
}

void GaeactorProcessorInterfaceInstance::refresh_events_by_cores(const TYPE_ULID &entityid, const H3INDEX &h3Indexsrc, const FLOAT64 &hgt, const transdata_entityposinfo& eninfo, IDENTIFI_EVENT_INFO& identifi_event_info)
{
    if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
    {
        if(m_pGaeactorProcessor_normal)
        {
            m_pGaeactorProcessor_normal->refresh_events_by_cores(entityid, h3Indexsrc, hgt, eninfo, identifi_event_info);
        }
    }
    else
    {
        if(m_pGaeactorProcessor)
        {
            m_pGaeactorProcessor->refresh_events_by_cores(entityid, h3Indexsrc, hgt, eninfo, identifi_event_info);
        }
    }
}

void GaeactorProcessorInterfaceInstance::refresh_events_by_sensors(const TYPE_ULID &sensorid, const TYPE_ULID &sensingmedia_id, const HEXIDX_HGT_ARRAY &hexidxslistret,IDENTIFI_EVENT_INFO& identifi_event_info)
{
    if(runningmode::RunningModeConfig::getInstance().get_MODE_USING_939_MODE())
    {
        if(m_pGaeactorProcessor_normal)
        {
            m_pGaeactorProcessor_normal->refresh_events_by_sensors(sensorid, sensingmedia_id, hexidxslistret, identifi_event_info);
        }
    }
    else
    {
        if(m_pGaeactorProcessor)
        {
            m_pGaeactorProcessor->refresh_events_by_sensors(sensorid, sensingmedia_id, hexidxslistret, identifi_event_info);
        }
    }
}
