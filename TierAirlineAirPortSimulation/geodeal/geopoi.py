...
for key, value in poi_pois.items():
    ...
    for point in value:        
        ...
        featuretmp={}
        featuretmp['type']='Feature'
        propertiestmp={}
        propertiestmp['poiKeyword']=key
        propertiestmp['marker-color']=color
        featuretmp['properties']=propertiestmp
        featuretmp['geometry']={}
        featuretmp['geometry']['type']='Point'
        coordinatestmp =[point.x, point.y]
        featuretmp['geometry']['coordinates']=coordinatestmp
        featurestmp.append(featuretmp)


poiGeoJSONtmp['type']='FeatureCollection'
poiGeoJSONtmp['features']=featurestmp
...
with open("Untitled-2.json", 'w') as f:
    json.dump(poiGeoJSONtmp, f, indent=4)
