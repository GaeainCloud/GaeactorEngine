...
traverse_dir()
{
	...
	do
		if [ -d $filepath/$file ] # if $filepath/$file is a directory
		then
			count=0
            for (( i = 0; i < ignoreDirListSize; i++ ))
			do
				if [[ $file != ${ignoreDirList[i]} ]] # charge by Regular expressions
				then
					let count++
				fi
			done
			...
			count=0
		else # if $filepath/$file is a file
